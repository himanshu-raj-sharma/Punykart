...
export const DonorWall: React.FC<DonorWallProps> = ({ donors, onAddBlessing }) => {
  ...
  const handleBlessingSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => setBlessingSubmitted(false), 3500);
  };
    ...
    <section id="donors" className="py-12 sm:py-16 bg-white border-b border-[#EAE5DD]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 pb-6 border-b border-slate-100">
          <div>
            <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#9F3D00] bg-amber-50 px-3 py-1 rounded-md mb-2">
            ...
            <p className="mt-2 text-sm sm:text-base text-slate-600">
              Honoring the generous devotees across India and abroad whose timely support is pulling Gaumata through trauma.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {displayedDonors.map((donor) => (
              <div
                key={donor.id}
                className="bg-[#FDFBF7] rounded-2xl border border-slate-200 p-5 flex flex-col justify-between hover:border-amber-300 hover:shadow-xs transition-all"
              >
                <div>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2.5">
                      <div className="w-9 h-9 rounded-full bg-amber-100 border border-amber-200 flex items-center justify-center font-display font-extrabold text-[#9F3D00] text-sm">
                        {donor.name.charAt(0)}
                      </div>
                      <div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-200/60 flex items-center justify-between text-[11px] text-slate-400">
                  ...
                  {donor.panNumberMasked && (
                    <span className="font-mono">PAN: {donor.panNumberMasked}</span>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="lg:col-span-4 bg-gradient-to-br from-amber-50 to-orange-50/70 rounded-2xl border border-amber-200 p-6 shadow-sm space-y-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Send your prayers and heartfelt blessings. Your holy words are read by our shelter caregivers as they tend to the recovering cows.
            </p>
            ...
            <form onSubmit={handleBlessingSubmit} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-white rounded-xl border border-slate-300 text-base sm:text-sm focus:ring-2 focus:ring-[#9F3D00] outline-none"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. May Gaumata heal completely and live in peace..."
                  className="w-full px-3.5 py-2.5 bg-white rounded-xl border border-slate-300 text-base sm:text-sm focus:ring-2 focus:ring-[#9F3D00] outline-none resize-none"
                />
              </div>
            ...
            {blessingSubmitted && (
              <div className="p-3 bg-emerald-100/80 border border-emerald-300 rounded-xl text-xs text-emerald-900 font-medium text-center animate-fade-in">
                🙏 Thank you! Your sacred prayer has been recorded.
              </div>
            )}
          </div>
