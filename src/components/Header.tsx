...
export const Header: React.FC<HeaderProps> = ({ onOpenTaxModal, onDonateClick }) => {
  ...
  return (
    <header className="header-shell sticky top-0 z-50 text-white border-b border-white/10 shadow-md transition-colors">
      ...
      <motion.div
        className="absolute bottom-0 left-0 right-0 h-[2.5px] bg-gradient-to-r from-amber-400 via-[#FF6B00] to-orange-500 origin-left z-50 pointer-events-none"
        style={{ scaleX }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-3">
        ...
        <nav className="hidden xl:flex items-center gap-5 text-xs lg:text-sm font-medium text-slate-200">
          ...
          <button
            ...
            className="flex items-center gap-1 bg-[#FF6B00] hover:bg-[#E05300] text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer"
          >
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {isLangOpen && (
              <div className="absolute right-0 mt-1 w-28 bg-[#001833] border border-slate-700 rounded-lg shadow-xl py-1 text-xs z-50">
                <button
                  type="button"
          ...
          <motion.button
            ...
            className="flex items-center gap-1.5 bg-[#FF6B00] hover:bg-[#E05300] active:scale-95 text-white text-xs sm:text-sm font-extrabold px-5 py-2.5 rounded-full transition-all shadow-md shadow-orange-950/40 cursor-pointer"
          >
        ...
        <div className="flex xl:hidden items-center gap-2">
          <motion.button
            ...
            type="button"
            onClick={onDonateClick}
            className="flex items-center gap-1 bg-[#FF6B00] hover:bg-[#E05300] text-white text-xs font-bold px-3 py-1.5 rounded-full"
          >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="xl:hidden overflow-hidden bg-[#001833] border-t border-slate-700 px-4 pt-3 pb-6 space-y-3 shadow-xl"
          >
            <div className="grid grid-cols-2 gap-2 pb-3 border-b border-slate-700 text-xs">
              <button
                type="button"
                onClick={() => { onOpenTaxModal(); setMobileMenuOpen(false); }}
                className="flex items-center justify-center gap-1.5 py-2 bg-[#FF6B00] hover:bg-[#E05300] text-white rounded-lg font-bold"
              >
                <FileText className="w-3.5 h-3.5" />
